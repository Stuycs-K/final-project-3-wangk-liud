...
def target_info():
    ...
    if request.form.get('default') == "y":
        ...
        update_config("cupp.cfg",specialYears,moreSpecialChars,numRange,wcRange,threshold)
        x_1 = noninteractive(targetName, targetSurname, targetNickname, targetBirthday, partnerName, partnerNickname, partnerBirthday, childName, childNickname, childBirthday, petName, company,keywords, specialChars, randomNumbers, leet)
        y_1 = generate_wordlist_from_profile(x_1)
        return render_template("response.html",wordlist=y_1)
